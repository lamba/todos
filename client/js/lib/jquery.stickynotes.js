(function(jQuery) {
	...
	console.log("loading stickynotes");

	jQuery.fn.stickyNotes = function(options) {
		console.log("jQuery.fn.stickynotes");
	...
	//used when user clicks "add note"?
	jQuery.fn.stickyNotes.createNote = function() {
		console.log('jQuery.fn.stickyNotes.createNote');
		var pos_x = 0;
		var pos_y = 0;		
    ...
    var _div_background = jQuery.fn.stickyNotes.createNoteBackground();
		...
		_div_wrap.draggable({
			...
			scroll: false, 
			start: function(event, ui){jQuery.fn.stickyNotes.startMovingNote(note_id)},
			drag: function(event, ui){jQuery.fn.stickyNotes.movingNote(note_id)},
			stop: function(event, ui){jQuery.fn.stickyNotes.movedNote(note_id)}
		}); 		
	...
	jQuery.fn.stickyNotes.stopEditing = function(note_id) {
		...
		$(_p_note_text)
			.dblclick(function() {jQuery.fn.stickyNotes.editNote(this);})
    ...
    jQuery(textarea).css({
	    ...
	    'height': jQuery("#note-" + note_id).height() - 5
    });
		jQuery("#note-" + note_id).find("textarea").focus();
		jQuery.fn.stickyNotes.setCurrentlyEditedNote(note_id);
	}
	...
	jQuery.fn.stickyNotes.renderNote = function(note) {
		var _p_note_text = $(document.createElement('p')).attr("id", "p-note-" + note.id).html(note.text);		
		var _div_note = jQuery(document.createElement('div')).addClass('jStickyNote');
    var _div_background = jQuery.fn.stickyNotes.createNoteBackground();
		...
		_div_wrap.draggable({
			containment: '#sticky-container', 
			scroll: false, 
			drag: function(event, ui){jQuery.fn.stickyNotes.movingNote(note.id)},
			stop: function(event, ui){jQuery.fn.stickyNotes.movedNote(note.id)}
		}); 
	...
	jQuery.fn.stickyNotes.movedNote = function(note_id) {
		console.log('stickyNotes.movedNote');
		var note = jQuery.fn.stickyNotes.getNote(note_id);
	...
	jQuery.fn.stickyNotes.movingNote = function(note_id) {
		console.log('stickyNotes.movingNote');
		var note = jQuery.fn.stickyNotes.getNote(note_id);
		note.pos_x=jQuery("#note-" + note_id).css("left").replace(/px/, "");
		note.pos_y=jQuery("#note-" + note_id).css("top").replace(/px/, "");
		console.log('jQuery.fn.stickyNotes.options.moveCallback='+jQuery.fn.stickyNotes.options.moveCallback);		
		if (/*typeof jQuery.fn.stickyNotes.options.moveCallback === 'function'*/ true) {
			console.log('stickyNotes.movedNote moveCallback === function');
			todos.noteMoving(note);
			//jQuery.fn.stickyNotes.options.moveCallback(note);
		}		
	};
	
	jQuery.fn.stickyNotes.startMovingNote = function(note_id) {
		console.log('stickyNotes.startMovingNote');
	};
  ...
  jQuery.fn.stickyNotes.createNoteBackground = function() {
    ...
    //} else {
        background = $(document.createElement('div')).addClass("background").html('<img src="js/lib/images/sticky-bg.png" class="stretch" style="margin-top:5px;" alt="" />');
    //}      
    return background;
  }
